...
| `hero_antibase` | 已完成第一版，已加入运行链路 | mode 4 反基地，位于功能包同级目录而非 `hero_aim`。仅在 mode4 消费 selected 基地图像，迁移旧工程中心裁剪/偏移、静态简化与拖影、低延迟 H.264、292B 数据分包、积压裁剪和自适应码率；发布 `/hero/aim/antibase/packets`、`/hero/aim/antibase/debug`，可选 `/hero/aim/antibase/visualization`。不使用检测、PnP、TF、弹道或 yaw/pitch 控制。 |
...
- mode4 串口协议决定：`AntiBasePacket` 保留源图像 `T0`，含 8B 小端 `sequence_id` 和 292B H.264 数据；`gimbal_driver` 是唯一串口拥有者，写为 5 个 `'#' + chunk_index + 60B + CRC16` 的 64B 帧，分片间隔 2 ms、逻辑包起始间隔 22 ms。其 `/hero/aim/antibase/tx_status` 的 `last_send_stamp` 是实际写出时刻 `Tsend`。mode4 自动发送码流，普通 14B 云台控制帧在该模式被抑制；切换模式会清空未发送包。虚拟串口中发送按逻辑成功计数，可用于离线链路验证。
- 后续迁移顺序：UDP/MQTT 反基地调试镜像 → 全系统 launch、rosbag 回归、部署验证。临时 `AimTarget` 与“公共 aim_controller”已移除，不得将新算法接回这类过渡接口。
