...
- 只有 aim8mm 具有 `timestamp_mode`、`timestamp_offset_sec` 与 `timing_topic`。默认 `host`，以 `GXDQBuf` 返回后、BGR 转换前记录的主机时刻作为图像近似 T0；视频输入只允许此模式。base 固定使用同一主机取帧时刻，不提供时间偏移或设备映射。
- aim8mm 的真实大恒输入可显式使用 `timestamp_mode: device`。驱动读取 SDK 时间戳频率，先以首帧建立设备计数到主机单调时间的相对映射，再用 0.02 系数低通修正。映射减少主机处理抖动，但其偏移仍包含相机到主机的传输延迟，不等同于精确曝光时刻。
- 仅 `/hero/camera/aim8mm/timing` 在有订阅者时发布 `CameraTiming`，可观察设备模式下的原始设备计数及频率、取图时间、发布前时间、最终图像时间、映射残差和重置次数；host/video 模式的设备字段为零。设备计数回退、频率变化、时间戳不递增或 ROS 时钟相对单调时钟跳变超过 100 ms 时，设备模式重置映射并丢弃跨边界帧；`use_sim_time` 不支持设备模式。
