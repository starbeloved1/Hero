...
{
  return target.id > 0U && target.tracking && target.converged && target.center_m.allFinite() &&
         target.velocity_mps.allFinite() && std::isfinite(target.yaw_rad) &&
...
{
  if (
    config_.system_response_time_sec < 0.0 || config_.fire_confirm_frames <= 0 ||
    config_.fast_fire_confirm_frames <= 0 || config_.high_acceleration_threshold_mps2 < 0.0 ||
    config_.stable_acceleration_threshold_mps2 < 0.0 || config_.acceleration_stable_frames <= 0)
  {
    throw std::invalid_argument("自瞄控制参数无效");
  }
}

std::optional<AutoAimResult> AutoAimer::aim(
  ...
  double gimbal_yaw_rad, double gimbal_pitch_rad)
{
  ...
  const double state_age_sec = std::max(0.0, control_stamp_sec - state_stamp_sec);
  ...
  for (int iteration = 0; iteration < 10; ++iteration) {
    predicted = predictTarget(*target, state_age_sec + config_.system_response_time_sec + flight_time_sec);
    selected = chooseArmor(predicted);
    if (!selected.has_value()) {
      return std::nullopt;
    }
    const auto next = aim_core::solveBallistics(
      config_.ballistics, selected->position_m.x(), selected->position_m.y(), selected->position_m.z());
    if (!next.has_value()) {
      return std::nullopt;
    }
    trajectory = *next;
  ...
  const auto command = smoother_.filter(trajectory.yaw_rad, trajectory.pitch_rad);
  ...
  const bool phase_ready = std::abs(selected->phase_error_rad) <= config_.coming_angle_rad;
  ...
  const bool fast = std::abs(predicted.angular_velocity_radps) > config_.high_spin_threshold_radps;
  const int required_frames = fast ? config_.fast_fire_confirm_frames : config_.fire_confirm_frames;
  ...
  result.aim_time_sec = control_stamp_sec + config_.system_response_time_sec + trajectory.flight_time_sec;
...
AutoTarget AutoAimer::predictTarget(const AutoTarget & target, double delay_sec)
{
  AutoTarget output = target;
  output.center_m += output.velocity_mps * std::max(0.0, delay_sec);
  output.yaw_rad = normalizeRadians(output.yaw_rad + output.angular_velocity_radps * std::max(0.0, delay_sec));
  return output;
}

std::array<AutoAimer::ArmorCandidate, 4> AutoAimer::buildArmors(const AutoTarget & target)
{
  std::array<ArmorCandidate, 4> armors{};
  for (int index = 0; index < 4; ++index) {
    const bool offset = index == 1 || index == 3;
    const double inward_yaw = normalizeRadians(target.yaw_rad + static_cast<double>(index) * kPi / 2.0);
    const double radius = offset ? target.radius_m + target.radius_offset_m : target.radius_m;
    armors[static_cast<std::size_t>(index)].index = index;
    armors[static_cast<std::size_t>(index)].inward_yaw_rad = inward_yaw;
    armors[static_cast<std::size_t>(index)].position_m = Eigen::Vector3d(
      target.center_m.x() - radius * std::cos(inward_yaw),
      target.center_m.y() - radius * std::sin(inward_yaw),
      offset ? target.center_m.z() + target.height_offset_m : target.center_m.z());
    const double surface_yaw = normalizeRadians(inward_yaw + kPi);
    const double to_shooter_yaw = std::atan2(
      -armors[static_cast<std::size_t>(index)].position_m.y(),
...
std::optional<AutoAimer::ArmorCandidate> AutoAimer::chooseArmor(const AutoTarget & target) const
{
  const auto armors = buildArmors(target);
  const bool fast = std::abs(target.angular_velocity_radps) > config_.high_spin_threshold_radps;
  ...
  std::optional<ArmorCandidate> best;
  for (const auto & armor : armors) {
    if (fast && std::abs(armor.phase_error_rad) > config_.coming_angle_rad) {
      continue;
    }
    // 高速旋转时只选择正朝可射窗口靠近的一面，避免刚离开正面的面板。
    if (fast && target.angular_velocity_radps > 0.0 && armor.phase_error_rad > config_.leaving_angle_rad) {
      continue;
    }
    if (fast && target.angular_velocity_radps < 0.0 && armor.phase_error_rad < -config_.leaving_angle_rad) {
      continue;
    }
    if (!best.has_value() || std::abs(armor.phase_error_rad) < std::abs(best->phase_error_rad)) {
      best = armor;
    }
  }
  return best;
...
const AutoTarget * AutoAimer::chooseTarget(const std::vector<AutoTarget> & targets)
{
  const auto valid = [](const AutoTarget & target) { return finiteTarget(target); };
  if (locked_target_id_.has_value()) {
    const auto it = std::find_if(targets.begin(), targets.end(), [this, &valid](const AutoTarget & target) {
  ...
  const auto it = std::min_element(targets.begin(), targets.end(), [&valid](const AutoTarget & lhs, const AutoTarget & rhs) {
...
double AutoAimer::updateAcceleration(const AutoTarget & target, double state_stamp_sec)
{
  const Eigen::Vector2d velocity(target.velocity_mps.x(), target.velocity_mps.y());
  double acceleration = 0.0;
  if (previous_velocity_mps_.has_value() && state_stamp_sec > previous_velocity_stamp_sec_ + 1e-6) {
