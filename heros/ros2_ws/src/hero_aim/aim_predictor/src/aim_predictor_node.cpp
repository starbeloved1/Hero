...
std::optional<Eigen::Vector3d> planeNormal(const geometry_msgs::msg::Quaternion & orientation)
{
...
: Node("aim_predictor_node")
{
  ...
  tracker_config_.min_consecutive_detections = get_parameter("min_consecutive_detections").as_int();
  tracker_config_.max_lost_frames = get_parameter("max_lost_frames").as_int();
  tracker_config_.low_speed_process_noise_xy = get_parameter("low_speed_process_noise_xy").as_double();
  tracker_config_.low_speed_process_noise_z = get_parameter("low_speed_process_noise_z").as_double();
  tracker_config_.low_speed_process_noise_yaw = get_parameter("low_speed_process_noise_yaw").as_double();
  tracker_config_.middle_speed_process_noise_xy = get_parameter("middle_speed_process_noise_xy").as_double();
  tracker_config_.middle_speed_process_noise_z = get_parameter("middle_speed_process_noise_z").as_double();
  tracker_config_.middle_speed_process_noise_yaw = get_parameter("middle_speed_process_noise_yaw").as_double();
  tracker_config_.high_speed_process_noise_xy = get_parameter("high_speed_process_noise_xy").as_double();
  tracker_config_.high_speed_process_noise_z = get_parameter("high_speed_process_noise_z").as_double();
  tracker_config_.high_speed_process_noise_yaw = get_parameter("high_speed_process_noise_yaw").as_double();
  ...
  tracker_config_.measurement_noise_yaw = get_parameter("measurement_noise_yaw").as_double();
  tracker_config_.measurement_noise_pitch = get_parameter("measurement_noise_pitch").as_double();
  ...
  tracker_config_.min_valid_radius_m = get_parameter("min_valid_radius_m").as_double();
  tracker_config_.max_valid_radius_m = get_parameter("max_valid_radius_m").as_double();
  ...
  if (
    target_frame_id_.empty() || camera_frame_id_.empty() || armor_pose_topic.empty() || gimbal_state_topic.empty() ||
  ...
  target_state_pub_ = create_publisher<hero_msgs::msg::TargetStateArray>(target_state_topic, qos);
  ...
  RCLCPP_INFO(
    get_logger(), "自瞄预测节点已启动：输入 %s，状态输出 %s，当前%s",
    armor_pose_topic.c_str(), target_state_topic.c_str(), enabled_ ? "启用" : "禁用");
}

void AimPredictorNode::receiveGimbalState(const hero_msgs::msg::GimbalState::ConstSharedPtr & message)
{
...
void AimPredictorNode::receiveArmorPoses(const hero_msgs::msg::ArmorPoseArray::ConstSharedPtr & message)
{
  if (
    !enabled_ || !latest_gimbal_state_.has_value() ||
  ...
  if (message->header.frame_id != target_frame_id_) {
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 2000, "自瞄预测仅接受 %s 坐标系位姿，当前为 %s",
      target_frame_id_.c_str(), message->header.frame_id.c_str());
    return;
  }
  updateTrackers(collectMeasurements(*message), stampToSeconds(message->header.stamp));
  publishStates(message->header);
}
  ...
  const std_msgs::msg::Header & measurement_header)
{
  try {
    const auto transform = tf_buffer_->lookupTransform(
      target_frame_id_, camera_frame_id_, rclcpp::Time(measurement_header.stamp),
    ...
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 2000, "预测器未找到同刻相机 TF %s <- %s：%s",
      target_frame_id_.c_str(), camera_frame_id_.c_str(), error.what());
    return std::nullopt;
  }
}

void AimPredictorNode::updateTrackers(const MeasurementsById & measurements, double stamp_sec)
{
  // 先预测到T0时刻，记一次丢失
  ...
  for (const auto & [id, target_measurements] : measurements) {
    ...
    const bool needs_initialize =
      tracker_it == trackers_.end() || !tracker_it->second.active() || tracker_it->second.diverged() ||
  ...
  for (auto it = consecutive_detection_counts_.begin(); it != consecutive_detection_counts_.end();) {
  ...
  for (auto it = trackers_.begin(); it != trackers_.end();) {
    if (it->second.diverged() || it->second.lostFrames() > tracker_config_.max_lost_frames) {
...
void AimPredictorNode::publishStates(const std_msgs::msg::Header & measurement_header)
{
