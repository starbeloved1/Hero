...
private:
  ...
  void processLoop();
  void publishDetections(const sensor_msgs::msg::Image::ConstSharedPtr &image,
                         const std::vector<class ArmorDetection> &detections);
  void
  publishVisualization(const sensor_msgs::msg::Image::ConstSharedPtr &image,
                       const std::vector<class ArmorDetection> &detections);
  ...
  std::atomic<uint8_t> gimbal_mode_{hero_msgs::msg::GimbalState::MODE_NORMAL};
  int target_color_{-1};
