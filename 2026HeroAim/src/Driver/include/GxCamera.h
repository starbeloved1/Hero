...
/// WhiteBalance
    ...
    public:
        ...
        // 采集不中断地修改手动曝光时间；结果经设备范围/步进裁剪后返回。
        bool adjustExposureUs(double delta_us, double* new_exposure_us = nullptr);

        // 运行期曝光控制的额外软件边界，仍会受相机自身范围约束。
        void setExposureControlRangeUs(double min_us, double max_us);

        // 在相机硬件侧固定连续采集频率；fps=0 会显式关闭硬件限帧。
        ...
        bool ProcGetImage(Mat *image, std::chrono::steady_clock::time_point *time_stamp);
    private:
    ...
    private:
        ...
        // 串行化采集与运行期特征写入，避免同一设备句柄的 SDK 调用并发。
        std::mutex device_mutex_;
        //pthread_mutex_t mutex;
        ExposureGain exposure_gain;                 ///< Camera exposure and gain param
        double exposure_control_min_us_ = 0.0;
        double exposure_control_max_us_ = 0.0;
        double acquisition_frame_rate_fps_ = 0.0;
