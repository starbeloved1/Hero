...
namespace driver {
    namespace {
        std::mutex gx_lib_mutex;
        int gx_lib_ref_count = 0;
    }
    ...
    GxCamera::~GxCamera() {
        ...
        //Release libary
        {
            std::lock_guard<std::mutex> lock(gx_lib_mutex);
            if (gx_lib_ref_count > 0) {
                --gx_lib_ref_count;
                if (gx_lib_ref_count == 0) {
                    emStatus = GXCloseLib();
                    if (emStatus != GX_STATUS_SUCCESS) {
...
/// Set camera exposure and gain params
    ...
    void GxCamera::setAcquisitionFrameRate(double fps)
    {
        acquisition_frame_rate_fps_ = std::max(0.0, fps);
    }
...
/// Set camera exposure and gain
    ...
    bool GxCamera::adjustExposureUs(double delta_us, double* new_exposure_us)
    {
        std::lock_guard<std::mutex> lock(device_mutex_);
        if (g_hDevice == nullptr || delta_us == 0.0) {
            return false;
        }

        GX_FLOAT_RANGE range{};
        GX_STATUS status = GXGetFloatRange(g_hDevice, GX_FLOAT_EXPOSURE_TIME, &range);
        if (status != GX_STATUS_SUCCESS) {
            GetErrorString(status);
            return false;
        }

        double current_us = exposure_gain.m_dExposureTime;
        status = GXGetFloat(g_hDevice, GX_FLOAT_EXPOSURE_TIME, &current_us);
        if (status != GX_STATUS_SUCCESS) {
            // 已知该相机配置为手动曝光；读回失败时以最近成功设置值为准。
            current_us = exposure_gain.m_dExposureTime;
        }

        const double min_us = std::max(range.dMin, exposure_control_min_us_);
        const double max_us = exposure_control_max_us_ > 0.0
            ? std::min(range.dMax, exposure_control_max_us_)
            : range.dMax;
        if (min_us > max_us) {
            return false;
        }
        double target_us = std::max(min_us, std::min(max_us, current_us + delta_us));
        if (range.bIncIsValid && range.dInc > 0.0) {
            target_us = range.dMin + std::round((target_us - range.dMin) / range.dInc) * range.dInc;
            target_us = std::max(min_us, std::min(max_us, target_us));
        }

        // 初始化时已关闭自动曝光；这里再次明确关闭，确保运行期写入立即生效。
        status = GXSetEnum(g_hDevice, GX_ENUM_EXPOSURE_AUTO, GX_EXPOSURE_AUTO_OFF);
        if (status != GX_STATUS_SUCCESS) {
            GetErrorString(status);
            return false;
        }
        status = GXSetFloat(g_hDevice, GX_FLOAT_EXPOSURE_TIME, target_us);
        if (status != GX_STATUS_SUCCESS) {
            GetErrorString(status);
            return false;
        }

        exposure_gain.m_bAutoExposure = false;
        exposure_gain.m_dExposureTime = target_us;
        if (new_exposure_us != nullptr) {
            *new_exposure_us = target_us;
        }
        return true;
    }

    void GxCamera::setExposureControlRangeUs(double min_us, double max_us)
    {
        exposure_control_min_us_ = std::max(0.0, min_us);
        exposure_control_max_us_ = std::max(exposure_control_min_us_, max_us);
    }
...
/// Main function to run the whole program
    GX_STATUS GxCamera::acquisitionStart()
    {
        ...
        // Mode4 需要稳定的输入节奏。该限制在相机端生效，而不是在应用层
        // sleep；SDK 不支持时保留原连续采集行为，不能因此阻止程序启动。
        if (acquisition_frame_rate_fps_ <= 0.0) {
            bool mode_supported = false;
            const GX_STATUS mode_query = GXIsImplemented(
                g_hDevice, GX_ENUM_ACQUISITION_FRAME_RATE_MODE, &mode_supported);
            if (mode_query == GX_STATUS_SUCCESS && mode_supported) {
                const GX_STATUS disable_status = GXSetEnum(
                    g_hDevice, GX_ENUM_ACQUISITION_FRAME_RATE_MODE,
                    GX_ACQUISITION_FRAME_RATE_MODE_OFF);
                if (disable_status == GX_STATUS_SUCCESS) {
                    std::cout << "[DaHenCamera] Acquisition FPS limit disabled" << std::endl;
                } else {
                    std::cerr << "[DaHenCamera] Failed to disable acquisition FPS limit" << std::endl;
                }
            }
        } else {
            bool mode_supported = false;
            ...
            const GX_STATUS mode_query = GXIsImplemented(
                g_hDevice, GX_ENUM_ACQUISITION_FRAME_RATE_MODE, &mode_supported);
            const GX_STATUS rate_query = GXIsImplemented(
                g_hDevice, GX_FLOAT_ACQUISITION_FRAME_RATE, &rate_supported);
            if (mode_query == GX_STATUS_SUCCESS && rate_query == GX_STATUS_SUCCESS &&
                mode_supported && rate_supported) {
                GX_FLOAT_RANGE range{};
                emStatus = GXGetFloatRange(
                    g_hDevice, GX_FLOAT_ACQUISITION_FRAME_RATE, &range);
                if (emStatus == GX_STATUS_SUCCESS) {
                    double requested = std::clamp(
                        acquisition_frame_rate_fps_, range.dMin, range.dMax);
                    if (range.bIncIsValid && range.dInc > 0.0) {
                        requested = range.dMin + std::round(
                            (requested - range.dMin) / range.dInc) * range.dInc;
                        requested = std::clamp(requested, range.dMin, range.dMax);
                    }
                    emStatus = GXSetEnum(g_hDevice, GX_ENUM_ACQUISITION_FRAME_RATE_MODE,
                                         GX_ACQUISITION_FRAME_RATE_MODE_ON);
                    if (emStatus == GX_STATUS_SUCCESS) {
                        emStatus = GXSetFloat(
                            g_hDevice, GX_FLOAT_ACQUISITION_FRAME_RATE, requested);
                    }
                    if (emStatus == GX_STATUS_SUCCESS) {
                        double applied = requested;
                        (void)GXGetFloat(
                            g_hDevice, GX_FLOAT_ACQUISITION_FRAME_RATE, &applied);
                        std::cout << "[DaHenCamera] Acquisition FPS requested="
                                  << acquisition_frame_rate_fps_
                                  << ", applied=" << applied << std::endl;
                    } else {
                        std::cerr << "[DaHenCamera] Failed to set acquisition FPS; "
                                  << "continuing without frame-rate control" << std::endl;
                        (void)GXSetEnum(g_hDevice, GX_ENUM_ACQUISITION_FRAME_RATE_MODE,
                                        GX_ACQUISITION_FRAME_RATE_MODE_OFF);
                    }
                } else {
                    std::cerr << "[DaHenCamera] Acquisition FPS range unavailable; "
                              << "continuing without frame-rate control" << std::endl;
                }
            } else {
                std::cerr << "[DaHenCamera] Acquisition FPS control unsupported; "
                          << "continuing without frame-rate control" << std::endl;
            }
        }
...
\return void*
\return bool  使用bool就只能转图像，不能用指针
*/
//-------------------------------------------------
    bool GxCamera::ProcGetImage(Mat *image, std::chrono::steady_clock::time_point *time_stamp)
    {
        std::lock_guard<std::mutex> lock(device_mutex_);
        ...
        PGX_FRAME_BUFFER pFrameBuffers[5];
        uint32_t ui32FrameCount = 0;
        if(*(threadParam->g_AcquisitionFlag))
        {
            // Get a frame from Queue
            emStatus = GXDQAllBufs(threadParam->m_hDevice, pFrameBuffers,5,&ui32FrameCount,1000);
...
//counter_.countBegin();
            ...
            // }
            if(emStatus != GX_STATUS_SUCCESS){
                std::cout << "GXCamera::ProcGetImage: cant borrow buffer";
                return false; 
            }

            if(ui32FrameCount==0){
                emStatus = GXQAllBufs(threadParam->m_hDevice);
                if (emStatus != GX_STATUS_SUCCESS)
                {
                    GetErrorString(emStatus);
                }
            ...
            {
                cv::Mat src;
                GX_FRAME_BUFFER *pFrameBuffer = nullptr;

                for(int i = static_cast<int>(ui32FrameCount) - 1; i >= 0; i--)
                {
                    const auto& buffer = pFrameBuffers[i];
                    if(buffer == nullptr || buffer->nStatus != GX_FRAME_STATUS_SUCCESS){
                        std::cout << "GXCamera::ProcGetImage: faile to get " << i << " image" << std::endl;
                        continue;
                    }
                    pFrameBuffer = buffer;
                    break;
                ...
                if(pFrameBuffer == nullptr){
                    std::cout << "GXCamera::ProcGetImage: all frames failed" << std::endl;
                    GXQAllBufs(threadParam->m_hDevice);
                    return false;
                }
                ...
                // 每台相机的 Bayer 排列由设备枚举返回，不能硬编码为 BAYERBG。
                // DxRaw8toRGB24Ex 明确要求 BGR 输出，才能直接交给 OpenCV 的 CV_8UC3。
                VxInt32 emDXStatus = DX_OK;
                emDXStatus = DxRaw8toRGB24Ex(
                    static_cast<unsigned char *>(pFrameBuffer->pImgBuf),
                    pBGRBuf,
                    pFrameBuffer->nWidth,
                    pFrameBuffer->nHeight,
                    RAW2RGB_NEIGHBOUR,
                    static_cast<DX_PIXEL_COLOR_FILTER>(g_i64ColorFilter),
                    false,
                    DX_ORDER_BGR);
                ...
                if (emDXStatus != DX_OK)
                {
                    ...
                    delete[] pBGRBuf;
                    pBGRBuf = NULL;
                    const auto return_result = GXQAllBufs(threadParam->m_hDevice);
                    if(return_result != GX_STATUS_SUCCESS)
                        std::cout <<"GXCamera::procGetImage: can not return buffer" << std::endl;
                    return false;
                }
                else
                {
